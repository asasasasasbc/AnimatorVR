...
public class OVRPlayerController : MonoBehaviour
{
    ...
    public bool pressGripToMove = false;
    public bool pressTriggerToUp = false;
    public float upSpeed = 1f;
    public Transform upTarget = null;
	/// <summary>
	...
	public virtual void UpdateMovement()
	{
		...
		if (EnableLinearMovement)
		{
            ...
            if (pressTriggerToUp) //When trigger is pressed and grip not pressed, go up/down
            {
                if (triggerPressed) // trigger pressed
                {
                    var up = primaryAxis.y;
                    upTarget.position += new Vector3(0, up * upSpeed * Time.deltaTime, 0);

                    primaryAxis.y = 0;
                }
            }

            if (pressGripToMove && pressTriggerToUp)
            {
                if (OVRInput.Get(OVRInput.Axis1D.PrimaryHandTrigger) < 0.5f && (!triggerPressed))
                {
                    primaryAxis *= 0;
                }
            } else if (pressGripToMove)
            {
